    ...
    fn translate_statement(&mut self, stmt: &Statement, trailing_expr: Expr) -> Result<Expr> {
        ...
        fn translate_assert(
            ...
            let expr = ptree_helpers::expr(
                Position::default(),
                ExprDesc::If(
                    Box::new(if assert.expected {
                        cond
                    } else {
                        ptree_helpers::eapply(Position::default(), NOT.clone(), cond)
                    }),
                    Box::new(UNIT_VALUE.clone()),
                    Box::new(translate_abort(on_failure)),
                ),
            );
...
static NOT: LazyLock<Expr> = LazyLock::new(|| {
    ptree_helpers::evar(
        Position::default(),
        ptree_helpers::qualid(Box::new([LIB_BOOL.1.as_str().into(), "bit_not".into()])),
    )
});
