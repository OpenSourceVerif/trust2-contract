...
fn remove_unused_locals_ullbc(body: &mut ullbc_ast::ExprBody) {
...
fn remove_unused_locals_llbc(body: &mut llbc_ast::ExprBody) {
    // Compute the set of used locals.
    // We always register the return variable and the input arguments.
    let mut visitor = LocalsUsageVisitor {
        used_locals: body
...
impl TransformPass for Transform {
    fn transform_ctx(&self, ctx: &mut TransformCtx) {
        ctx.for_each_fun_decl(|_ctx, fun| match &mut fun.body {
            Body::Unstructured(body) => remove_unused_locals_ullbc(body),
            Body::Structured(body) => remove_unused_locals_llbc(body),
            _ => {}
        });
    }
}
